import { WMOCodesMeaning, type WeatherCodeDetails, WeatherCodeDetailsMap, WeatherCodeKey } from "$lib/models";

export class WeatherCodeDetailsFactory {

  public create(wmoCode: number): WeatherCodeDetails {
    const { icon, color, label } = this.getWeatherCodeDetails(wmoCode);
    return {
      wmoCode,
      icon,
      color,
      label,
      wmoCodeDescription: WMOCodesMeaning[wmoCode]
    };
  }

  private getWeatherCodeDetails(wmoCodeNumber: number): WeatherCodeDetails {
